function yprime = syst_odes_wSocCoupling_YJM(t, x_vec, parameters_, temp_history, xP0, xR0)
	...
	kappa = parameters_.kappa; % social learning rate 			DOESN'T EXIST ANYMORE
	% beta  = parameters_.beta;  % net cost of being a mitigator	DOESN'T EXIST ANYMORE
	...
	if (temp_pred_ON == 1) && (t>=2014)
		...
		temp_x0_ = [xP0, xR0];
		dPdt = replicator_equation(proportions, meeting_rates, fitnesses, homophily, kappa);
		y1 = dPdt(1); % poor
		y2 = dPdt(2); % rich 
	end

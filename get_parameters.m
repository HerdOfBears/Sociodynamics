function parameters_ = get_parameters(random_yes_no)
    ...
    if ~random_yes_no
        ...
        parameters_given = [x_0, C_at0, C_oc0, C_veg0, C_so0, T_0];
        ...
        kappa = random(pdkappa, 1,1);    % (0.02, 0.05, 0.2) social learning rate
        
        pdbeta = makedist('Triangular', 'a', 0.5, 'b', 1, 'c', 1.5);
        beta  = random(pdbeta,1,1);    % (0.5, 1, 1.5) net cost of being a mitigator
        
        pddelta = makedist('Triangular', 'a', 0.5, 'b', 1, 'c', 1.5);
        delta = random(pddelta, 1,1);    % (0.5, 1, 1.5) strength of social norms

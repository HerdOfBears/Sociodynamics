function parameters =  params4SensiAna_YJM_perc(percentage_, vary_prop_)
		...
		%%%%%%%%%%%%%% YJM MODEL PARAMS
		if vary_prop_
			temp_.prop_R0 = [0.10, 0.25, 0.40];
		else
			temp_.prop_R0 = 0.25;
		end
		lower_ = 1 - percentage_;
		...
		% temp_.omega_R = 3.60; % derived from above two assumptions: I_R = 4 * (0.45/0.55) * I_P
		% temp_.omega_P = 2.4;
		temp_.omega_R = [lower_*5.0, 5.0, upper_*5.0];
		temp_.omega_P = [lower_*3.5, 3.5, upper_*3.5];
		
		% FOR WHEN RESOURCE DIFFERENCE IS NOT NORMALIZED
		% temp_.Omega = [lower_.*4.5,4.5,upper_.*4.5];
		% temp_.Td_c =  [lower_.*1.7, 1.7, upper_.*1.7];

		% FOR WHEN RESOURCE DIFFERENCE IS NORMALIZED
		temp_.Omega = [lower_*3,3,upper_*3];
		...
		% If we are varying the 
		if vary_prop_

		else
			temp_.x0_ = [0.01, 0.05, 0.1];
			% temp_.xP0   = x0_ .* (1 - temp_.prop_R0);
			% temp_.xR0   = x0_ .* temp_.prop_R0;
		end

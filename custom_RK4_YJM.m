function y = custom_RK4_YJM(odefun, tspan, y0, params_given, temp_history, vec_proportions)
...
% Y0     contains the intial conditions for the system variables
    ...
    xP0 = vec_proportions(1);
    xR0 = vec_proportions(2);
    ...
    for i = 1:(N-1)
        ...
        k(1, :) = odefun(t(i), y(i,:), params_given, T_prev, xP0, xR0);        
        k(2, :) = odefun(t(i) + (h/2), y(i,:) + (h/2)*k(1,:), params_given, T_prev, xP0, xR0);        
        k(3, :) = odefun(t(i) + (h/2), y(i,:) + (h/2)*k(2,:), params_given, T_prev, xP0, xR0);
        %{
        if t(i)>2014%i>t_p
            disp(i+1-1000)
        ...
        k(4, :) = odefun(t(i) + h, y(i,:) + h*k(3,:), params_given, T_prev, xP0, xR0);
